...
import type { GroupedBarChartData, GroupedBarChartOptions } from './groupedBarChart';
    ...
    props.data.groups.forEach((group, groupIndex) => {
        ...
        if (config.showLabels) {
            categoryGroups
                ...
                .text((d, i) => group.data[i])
                .attr('x', x1(group.name)! + x1.bandwidth() / 2)
                .attr('y', (d, i) => y(group.data[i]) - (config.labelOffset ?? 5))
