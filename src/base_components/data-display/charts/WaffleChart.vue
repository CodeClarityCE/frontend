...
function createData(): void {
  ...
  const colorsDefinedInData = props.data.every(
    (entry) => entry.color !== undefined,
  );
  ...
  for (const entry of normalizedData) {
    ...
    if (entry.value < 0.01) {
      ...
      for (
        ...
        i <
        Math.round(
          Number.parseFloat(entry.value.toPrecision(2)) * nmbOfSquares.value,
        );
        i++
      ) {
...
<template>
  <div
    ref="chartContainer"
    class="flex flex-row gap-8 justify-center items-start h-full w-full"
  >
    <!-- Waffle chart -->
    <div
      class="grid gap-0"
