...
const projectRepository: LicensePolicyRepository =
...
const totalPages: Ref<number> = ref(
  Math.ceil(license_policies_list.value.length / 10),
);
    ...
    try {
      ...
      if (selected_license_policy_object.value)
        selected_license_policy.value =
          selected_license_policy_object.value?.content;
    } catch (err) {
  ...
  <div>
    <div class="text-2xl font-medium mb-8">
      ...
      <div class="text-sm text-gray-500 mt-1">
        Using a license policy you can tell our analyzers which licenses are not
        allowed to appear within your projects. If we find such a license, we
        prioritize them so you can quickly see if any forbidden licenses were
    ...
    <LoadingContainer ref="license_policies_list_loading_ref">
      <template #content>
        <div
          v-if="license_policies_list.length > 0"
          class="license-policies-list-wrapper"
        >
          <!-- No Policy Option -->
          <div
        ...
        <div v-else class="license-policies-list-wrapper">
          ...
          >
            <div class="text-blue-800">
              💡 You have no license policies configured.
            </div>
            <div class="text-sm text-blue-700 mt-2">
              ...
              if you want to restrict specific licenses, but it's completely
              optional.
            </div>
          </div>
        </div>
