...
export class AnalyzerRepository extends BaseRepository {
  async getAnalyzers(
    options: GetAnalyzersRequestOptions,
  ): Promise<PaginatedResponse<Analyzer>> {
    ...
    return Entity.unMarshal<PaginatedResponse<Analyzer>>(
      response,
      PaginatedResponse<Analyzer>,
    );
  }

  async getAnalyzer(
    options: GetAnalyzerRequestOptions,
    ...
    return Entity.unMarshal<DataResponse<Analyzer>>(
      response,
      DataResponse<Analyzer>,
    );
  }
  ...
  async deleteAnalyzer(
    options: DeleteAnalyzerRequestOptions,
  ): Promise<NoDataResponse> {
    const RELATIVE_URL = `/org/${options.orgId}/analyzers/${options.analyzer_id}`;
