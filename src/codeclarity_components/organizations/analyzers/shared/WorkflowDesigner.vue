...
function addNodeToGraph(plugin: Plugin) {
    ...
    const currentNodes = allNodes.filter((n): n is AnalyzerNode => n.type === 'analyzer');
...
function buildPluginWithDependencies(
    ...
    plugin.depends_on.forEach((dependencyName) => {
        const dependencyPlugin = availablePlugins.find((p) => p.name === dependencyName);
        if (dependencyPlugin) {
...
function deleteNode(node: any) {
    ...
    const filteredEdges = props.edges.filter((e) => e.source !== node.id && e.target !== node.id);

    const remainingNodes = filteredNodes.filter((n): n is AnalyzerNode => n.type === 'analyzer');
    ...
    <div class="bg-white border border-slate-200/60 rounded-lg p-6">
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-lg font-semibold text-theme-black">Workflow Designer</h3>
            </div>
                ...
                <div class="absolute bottom-4 left-4 z-10">
                    ...
                    >
                        <div class="flex items-center gap-6 text-xs text-theme-gray">
                            ...
                            <span class="flex items-center gap-1">
                                <Icon icon="solar:link-bold" class="w-3 h-3 text-theme-primary" />
