...
describe.skip("ResultsCodeQL.vue", () => {
  ...
  it('shows "Well done" alert when no CodeQL results', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(wrapper.find('[data-testid="alert-description"]').exists()).toBe(
        true,
      );
    });
    ...
    expect(alertDescription.text()).toBe(
  ...
  it("renders location badges correctly", async () => {
    ...
    await vi.waitFor(() => {
      expect(wrapper.findAll('[data-testid="badge"]').length).toBeGreaterThan(
        0,
      );
    });
    ...
    expect(
      wrapper.findAll('[data-testid="badge"]').length,
  ...
  it("handles API error gracefully", async () => {
    ...
    mockResultsRepository.getResultByType.mockRejectedValue(
      new Error("API Error"),
    );
  ...
  it("has correct grid layout for results", async () => {
    ...
    await vi.waitFor(() => {
      expect(
        wrapper.find(".grid.md\\:grid-cols-2.xl\\:grid-cols-3.gap-2").exists(),
      ).toBe(true);
    });
  });
