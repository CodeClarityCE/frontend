...
describe.skip("LicensesComponent.vue", () => {
  ...
  it("calls API with correct parameters on init", async () => {
    ...
    mockResultsRepository.getLicenses.mockResolvedValue(
      createMockResponse(mockLicenses),
    );
  ...
  it("updates state after successful API call", async () => {
    ...
    const mockLicenses = [
      createMockLicense(),
      createMockLicense({ id: "Apache-2.0" }),
    ];
  ...
  it("throws error when no default org", async () => {
    ...
    await expect((wrapper.vm as any).init()).rejects.toThrow(
