...
vi.mock("@/base_components/data-display/charts/RadarChart.vue", () => ({
  default: {
    name: "RadarChart",
    template:
      '<div data-testid="radar-chart" :data-chart-id="id">Radar Chart</div>',
...
vi.mock("@/base_components/data-display/charts/GroupedBarChart.vue", () => ({
  default: {
    ...
    template:
      '<div data-testid="grouped-bar-chart" :data-chart-id="id">Bar Chart</div>',
    props: ["id", "data", "options"],
  },
}));
...
describe.skip("PatchingSummary.vue", () => {
  ...
  describe("Component Rendering", () => {
    it("should render the main summary wrapper", () => {
      ...
      expect(
        wrapper.find(".summary-wrapper.findings-summary-wrapper").exists(),
      ).toBe(true);
    });
    ...
    it("should display loading state initially", () => {
      wrapper = createWrapper();
      expect(
        wrapper.findAllComponents({ name: "TextLoader" }).length,
      ).toBeGreaterThan(0);
    });
    ...
    it("should render charts after data loads", async () => {
      ...
      expect(wrapper.findComponent({ name: "GroupedBarChart" }).exists()).toBe(
        true,
      );
  ...
  describe("Error Handling", () => {
    it("should display error alert when API fails", async () => {
      const consoleErrorSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
      mockResultsRepository.getPatchesStat.mockRejectedValue(
        new Error("API Error"),
      );
