...
vi.mock("@/shadcn/ui/card/CardContent.vue", () => ({
  default: {
    name: "CardContent",
    template:
      '<div data-testid="card-content" class="card-content"><slot></slot></div>',
    props: ["class"],
  },
}));
...
vi.mock("@/shadcn/ui/tabs/TabsList.vue", () => ({
  default: {
    name: "TabsList",
    template:
      '<div data-testid="tabs-list" class="tabs-list"><slot></slot></div>',
    props: ["class"],
  },
}));
...
describe("PatchComponent.vue", () => {
  ...
  describe("Component Rendering", () => {
    it("should render the main card container", () => {
      ...
      expect(wrapper.findComponent({ name: "CardContent" }).exists()).toBe(
        true,
      );
    });
  ...
  describe("Patch Status Display", () => {
    it("should display full patch status", () => {
      ...
      const fullPatchIcon = wrapper.findAll(
    ...
    it("should display partial patch status", () => {
      ...
      const partialPatchIcon = wrapper.findAll(
        '[data-icon="bi:shield-fill-minus"]',
      );
    ...
    it("should display no patch status", () => {
      ...
      const noPatchIcon = wrapper.findAll(
        '[data-icon="bi:shield-fill-exclamation"]',
      );
  ...
  describe("Patch Information Components", () => {
    it("should render PatchInformation components for unpatchable items", () => {
      ...
      const patchInfoComponents = wrapper.findAllComponents({
        name: "PatchInformation",
      });
  ...
  describe("Layout and Styling", () => {
    it("should apply correct flex layout classes", () => {
      ...
      expect(
        wrapper.find(".flex.flex-row.items-center.gap-8.font-bold").exists(),
      ).toBe(true);
    });
  ...
  describe("Component Integration", () => {
    it("should properly integrate with shadcn components", () => {
      ...
      expect(wrapper.findComponent({ name: "TabsList" }).exists()).toBe(true);
      expect(wrapper.findAllComponents({ name: "TabsTrigger" })).toHaveLength(
        2,
      );
      expect(wrapper.findAllComponents({ name: "TabsContent" })).toHaveLength(
        2,
      );
    });
