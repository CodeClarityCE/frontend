...
vi.mock("@/shadcn/ui/breadcrumb/Breadcrumb.vue", () => ({
  default: {
    name: "Breadcrumb",
    template:
      '<nav data-testid="breadcrumb" class="breadcrumb"><slot></slot></nav>',
  },
}));
...
vi.mock("@/shadcn/ui/breadcrumb/BreadcrumbList.vue", () => ({
  default: {
    name: "BreadcrumbList",
    template:
      '<ol data-testid="breadcrumb-list" class="breadcrumb-list"><slot></slot></ol>',
  },
}));
...
vi.mock("@/shadcn/ui/breadcrumb/BreadcrumbItem.vue", () => ({
  default: {
    name: "BreadcrumbItem",
    template:
      '<li data-testid="breadcrumb-item" class="breadcrumb-item"><slot></slot></li>',
  },
}));
...
vi.mock("@/shadcn/ui/breadcrumb/BreadcrumbPage.vue", () => ({
  default: {
    name: "BreadcrumbPage",
    template:
      '<span data-testid="breadcrumb-page" class="breadcrumb-page"><slot></slot></span>',
  },
}));
...
vi.mock("@/shadcn/ui/breadcrumb/BreadcrumbSeparator.vue", () => ({
  default: {
    ...
    template:
      '<span data-testid="breadcrumb-separator" class="breadcrumb-separator">/</span>',
  },
}));
...
describe("PatchInformation.vue", () => {
  ...
  describe("Component Rendering", () => {
    it("should render the main container", () => {
      ...
      expect(
        wrapper.find(".flex.gap-2.items-center.justify-between").exists(),
      ).toBe(true);
    });
    ...
    it("should render breadcrumb navigation", () => {
      ...
      expect(wrapper.findComponent({ name: "BreadcrumbList" }).exists()).toBe(
        true,
      );
    });
    ...
    it("should render patched version section", () => {
      ...
      expect(wrapper.findComponent({ name: "SemverToString" }).exists()).toBe(
        true,
      );
    });
  });
  ...
  describe("Breadcrumb Navigation", () => {
    it("should render root breadcrumb item", () => {
      ...
      const breadcrumbItems = wrapper.findAllComponents({
        name: "BreadcrumbItem",
      });
    ...
    it("should render breadcrumb separators", () => {
      ...
      const separators = wrapper.findAllComponents({
        name: "BreadcrumbSeparator",
      });
    ...
    it("should render breadcrumb pages correctly", () => {
      ...
      const breadcrumbPages = wrapper.findAllComponents({
        name: "BreadcrumbPage",
      });
  ...
  describe("Layout and Styling", () => {
    ...
    it("should have proper component structure", () => {
      ...
      expect(container.findComponent({ name: "Breadcrumb" }).exists()).toBe(
        true,
      );
  ...
  describe("Component Integration", () => {
    it("should properly integrate with breadcrumb components", () => {
      ...
      expect(
        wrapper.findAllComponents({ name: "BreadcrumbItem" }).length,
      ).toBeGreaterThan(0);
      expect(
        wrapper.findAllComponents({ name: "BreadcrumbPage" }).length,
      ).toBeGreaterThan(0);
      expect(
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle very long vulnerability IDs", () => {
      const longVulnPatch = {
        ...
        Vulnerability: {
          VulnerabilityId:
            "CVE-2021-1234567890-VERY-LONG-VULNERABILITY-IDENTIFIER",
          Severity: {
            Impact: "CRITICAL",
          },
        },
