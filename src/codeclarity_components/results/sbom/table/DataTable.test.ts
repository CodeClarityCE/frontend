...
vi.mock("@/shadcn/ui/table", () => ({
  ...
  TableCell: {
    name: "TableCell",
    template:
      '<td data-testid="table-cell" :colspan="colSpan"><slot></slot></td>',
    props: ["colSpan"],
  },
  TableHead: {
    name: "TableHead",
  ...
  TableRow: {
    name: "TableRow",
    template:
      '<tr data-testid="table-row" :class="$attrs.class"><slot></slot></tr>',
  },
}));
...
vi.mock("@/shadcn/ui/button", () => ({
  Button: {
    name: "Button",
    template:
      '<button data-testid="button" :class="variant"><slot></slot></button>',
    props: ["variant", "size"],
  },
}));
...
vi.mock("@/shadcn/ui/select", () => ({
  ...
  SelectItem: {
    name: "SelectItem",
    template:
      '<div data-testid="select-item" @click="$emit(\'click\')">{{ value }}</div>',
    props: ["value"],
    emits: ["click"],
  },
  SelectTrigger: {
    name: "SelectTrigger",
    template:
      '<div data-testid="select-trigger" @click="$emit(\'click\')"><slot></slot></div>',
    emits: ["click"],
  },
  SelectValue: {
    name: "SelectValue",
    template:
      '<span data-testid="select-value">{{ placeholder || modelValue }}</span>',
    props: ["placeholder", "modelValue"],
  },
}));
...
describe("DataTable.vue", () => {
  ...
  describe("Component Rendering", () => {
    ...
    it("should render the header section with search and filters", () => {
      ...
      expect(wrapper.find(".flex.flex-col.sm\\:flex-row.gap-4").exists()).toBe(
        true,
      );
      expect(wrapper.findComponent({ name: "Input" }).exists()).toBe(true);
      expect(
        wrapper.findAllComponents({ name: "DropdownMenu" }).length,
      ).toBeGreaterThan(0);
    });
    ...
    it("should render the table", () => {
      ...
      expect(wrapper.findComponent({ name: "TableHeader" }).exists()).toBe(
        true,
      );
    ...
    it("should render pagination controls", () => {
      ...
      const paginationSection = wrapper.find(
        ".flex.items-center.justify-between.py-4.px-2",
      );
      expect(paginationSection.exists()).toBe(true);
      expect(paginationSection.findComponent({ name: "Select" }).exists()).toBe(
        true,
      );
    });
  });
  ...
  describe("Quick Filters", () => {
    ...
    it("should show filter options", () => {
      ...
      const filterItems = wrapper.findAll(
  ...
  describe("Pagination", () => {
    ...
    it("should have page size options", () => {
      ...
      const pageSizeOptions = wrapper
        ...
        .filter((item: any) =>
          ["10", "15", "20", "30", "40", "50"].includes(item.text()),
        );
  ...
  describe("Interaction Behavior", () => {
    ...
    it("should have responsive classes", () => {
      ...
      expect(wrapper.find(".flex.flex-col.sm\\:flex-row.gap-3").exists()).toBe(
        true,
      );
    });
