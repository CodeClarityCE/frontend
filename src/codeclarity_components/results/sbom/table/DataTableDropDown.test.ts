...
const mockRouterLink = {
  name: "RouterLink",
  template:
    '<a data-testid="router-link" :href="to.name" v-bind="$attrs"><slot></slot></a>',
  props: ["to", "title"],
  inheritAttrs: true,
};
...
describe("DataTableDropDown.vue", () => {
  ...
  describe("Component Rendering", () => {
    ...
    it("should render separator", () => {
      ...
      const separator = wrapper.findComponent({
        name: "DropdownMenuSeparator",
      });
  ...
  describe("URL Parameters Handling", () => {
    ...
    it("should handle missing URL parameters", () => {
      // Update window.location.search to have no params
  ...
  describe("Copy Functionality", () => {
    it("should copy vulnerabilities when they exist", async () => {
      ...
      const copyItem = wrapper.findAllComponents({
        name: "DropdownMenuItem",
      })[0];
