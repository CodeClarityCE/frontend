...
vi.mock("./VulnDetails/VulnSummaryContent.vue", () => ({
  default: {
    ...
    template:
      '<div data-testid="vuln-summary-content">VulnSummaryContent</div>',
  },
}));
...
vi.mock("./VulnDetails/VulnerabilitySeverities.vue", () => ({
  default: {
    ...
    template:
      '<div data-testid="vulnerability-severities">VulnerabilitySeverities</div>',
  },
}));
...
vi.mock("@/base_components/ui/modals/CenteredModal.vue", () => ({
  default: {
    name: "CenteredModal",
    template:
      '<div data-testid="centered-modal"><slot name="content"></slot></div>',
    methods: {
      show: vi.fn(),
      toggle: vi.fn(),
    },
...
vi.mock("@/base_components/ui/modals/PositionedModal.vue", () => ({
  default: {
    ...
    template:
      '<div data-testid="positioned-modal"><slot name="content"></slot></div>',
    methods: {
      toggle: vi.fn(),
    },
  },
...
describe.skip("VulnDetails.vue", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(ResultsRepository).mockImplementation(
      () => mockResultsRepository,
    );
  ...
  it("renders loading state initially", () => {
    ...
    expect(wrapper.find('[data-testid="vuln-details-loader"]').exists()).toBe(
      true,
    );
  });
  ...
  it("gets security score variant correctly", () => {
    ...
    expect((wrapper.vm as any)["getSecurityScoreVariant"](mockVuln)).toBe(
      "success",
    );
  ...
  it("gets medium/low count description correctly", () => {
    ...
    expect((wrapper.vm as any)["getMediumLowCount"](mockVuln)).toBe(
      "1 medium, 0 low",
    );
  ...
  it("opens CVSS modal correctly", () => {
    ...
    expect((wrapper.vm as any)["cvss_info"]).toEqual({
      AV: { full_name: "Attack Vector" },
    });
  });
  ...
  it("sets chart version based on CVSS data", async () => {
    ...
    await vi.waitFor(() => {
      expect(
        (wrapper2.vm as unknown as Record<string, unknown>)["chart_version"],
      ).toBe("cvss3");
    });
    ...
    await vi.waitFor(() => {
      expect(
        (wrapper3.vm as unknown as Record<string, unknown>)["chart_version"],
      ).toBe("cvss2");
    });
  });
});
