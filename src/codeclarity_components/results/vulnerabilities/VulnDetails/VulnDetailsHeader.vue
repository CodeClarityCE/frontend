...
<template>
    <div class="header flex flex-col gap-5 bg-white shadow-md rounded-lg p-6">
        ...
        <div class="text-gray-600">
            ...
            <div v-if="!finding.vulnerability_info.version_info.source_comparison || finding.vulnerability_info.version_info.source_comparison.agree">
            ...
            <div v-else class="space-y-3">
                <div class="flex gap-2 items-center text-orange-600 bg-orange-50 p-3 rounded-lg border-l-4 border-orange-400">
                ...
                <div class="grid md:grid-cols-2 gap-4 ml-2">
                    <!-- NVD Source -->
                    <div v-if="finding.vulnerability_info.version_info.source_comparison.nvdReason" 
                        ...
                        <details class="mt-2">
                            ...
                            <div class="mt-2 p-2 bg-blue-100 rounded text-xs text-blue-800 font-mono whitespace-pre-line">
                                {{ finding.vulnerability_info.version_info.source_comparison.nvdAllVersions.replace(/, /g, ',\n') }}
                            </div>
                        </details>
                    </div>
                    
                    <!-- OSV Source -->
                    <div v-if="finding.vulnerability_info.version_info.source_comparison.osvReason"
                        ...
                        <!-- Expandable technical details -->
                        <details class="mt-2">
                            <summary class="cursor-pointer text-xs text-green-600 hover:text-green-800 font-medium">
                                View all affected versions
                            </summary>
                            <div class="mt-2 p-2 bg-green-100 rounded text-xs text-green-800 font-mono whitespace-pre-line">
                                {{ finding.vulnerability_info.version_info.source_comparison.osvAllVersions.replace(/, /g, ',\n') }}
                            </div>
                        </details>
                    </div>
                </div>
                ...
                <div class="bg-yellow-50 p-3 rounded-lg border-l-4 border-yellow-400">
                    ...
                    <div class="text-sm text-yellow-700">
                        Since sources disagree on your version <strong>{{ finding.dependency_info?.version }}</strong>, 
                        review the explanations above and check the official advisory links to determine your actual risk.
                    </div>
                </div>
            </div>
            <div
