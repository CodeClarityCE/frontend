...
describe("VulnerabilitiesInfo.vue", () => {
  ...
  describe("Component Rendering", () => {
    it("should render the main container", () => {
      ...
      expect(
        wrapper
          .find(".flex.flex-row.justify-center.items-center.gap-2")
          .exists(),
      ).toBe(true);
    });
  ...
  describe("Data Processing", () => {
    ...
    it("should map severity levels correctly", () => {
      ...
      const criticalItem = chartData.find(
        (item: any) => item.label === "Critical",
      );
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle large severity counts", () => {
      ...
      expect(
        chartData.find((item: any) => item.label === "Critical").count,
      ).toBe(9999);
      expect(chartData.find((item: any) => item.label === "High").count).toBe(
        8888,
      );
      expect(chartData.find((item: any) => item.label === "Medium").count).toBe(
        7777,
      );
      expect(chartData.find((item: any) => item.label === "Low").count).toBe(
        6666,
      );
      expect(chartData.find((item: any) => item.label === "None").count).toBe(
        5555,
      );
    });
