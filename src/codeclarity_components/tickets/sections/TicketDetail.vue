...
const availableSyncProviders = computed(() => {
  const linkedProviders = new Set(
    props.ticket.external_links.map((link) => link.provider),
  );
...
const epssFormatted = computed(() => {
  ...
  return {
    ...
    scoreRaw: score * 100,
    percentile:
      percentile !== undefined ? `${(percentile * 100).toFixed(1)}%` : null,
...
function getCvssValueColor(value: string | undefined): string {
  ...
  if (highRisk.some((h) => value.toUpperCase().includes(h)))
    return "text-red-600";
  if (lowRisk.some((l) => value.toUpperCase().includes(l)))
        ...
        >
          ...
          <div>
            ...
            <p class="text-sm opacity-90">
              {{ riskBannerConfig.recommendation }}
            </p>
          </div>
        </div>
                ...
                >
                  <Icon
                    ...
                    :class="{
                      ...
                      'text-gray-400': !['CLICKUP', 'JIRA', 'LINEAR'].includes(
                        link.provider,
                      ),
                    }"
                  />
