...
export interface CreateTicketRequestOptions extends AuthRepoMethodPostRequestOptions<CreateTicketRequest> {
  orgId: string;
}

export interface UpdateTicketRequestOptions extends AuthRepoMethodPatchRequestOptions<UpdateTicketRequest> {
  orgId: string;
  ticketId: string;
}

export interface BulkUpdateTicketsRequestOptions extends AuthRepoMethodPatchRequestOptions<BulkUpdateTicketsRequest> {
  orgId: string;
}
...
export interface CheckDuplicateRequestOptions extends AuthRepoMethodPostRequestOptions<CheckDuplicateRequest> {
  orgId: string;
}
...
export interface ConfigureClickUpRequestOptions extends AuthRepoMethodPostRequestOptions<ConfigureClickUpRequest> {
  orgId: string;
}
...
export interface CreateHierarchyItemRequestOptions extends AuthRepoMethodPostRequestOptions<{
  name: string;
}> {
  orgId: string;
  provider: ExternalTicketProvider;
...
export interface BulkSyncRequestOptions extends AuthRepoMethodPostRequestOptions<{
  ticket_ids: string[];
}> {
  orgId: string;
  provider: ExternalTicketProvider;
...
export interface BulkSyncFromExternalRequestOptions extends AuthRepoMethodPostRequestOptions<{
  ticket_ids: string[];
}> {
  orgId: string;
}
...
export interface ExchangeClickUpOAuthCodeRequestOptions extends AuthRepoMethodPostRequestOptions<{
  code: string;
  redirect_uri: string;
}> {
  orgId: string;
...
export class TicketsRepository extends BaseRepository {
   ...
   */
  async getTickets(
    options: GetTicketsRequestOptions,
    ...
    if (options.filters) {
      ...
      if (options.filters.has_external_link !== undefined) {
        queryParams.has_external_link = options.filters.has_external_link
          ? "1"
          : "0";
      }
    ...
    paginatedData.data = Entity.unMarshalMany<TicketSummary>(
      paginatedData.data,
      TicketSummary,
    );
  ...
  async getTicketEvents(
    ...
    paginatedData.data = Entity.unMarshalMany<TicketEvent>(
      paginatedData.data,
      TicketEvent,
    );
    return paginatedData;
  ...
  async getDashboardStats(
    ...
    data.data = Entity.unMarshal<TicketDashboardStats>(
      data.data,
      TicketDashboardStats,
    );
    return data;
   ...
   */
  async createTicket(
    options: CreateTicketRequestOptions,
   ...
   */
  async updateTicket(
    options: UpdateTicketRequestOptions,
   ...
   */
  async deleteTicket(
    options: DeleteTicketRequestOptions,
  ): Promise<NoDataResponse> {
    const RELATIVE_URL = `/org/${options.orgId}/tickets/${options.ticketId}`;
  ...
  async deleteIntegration(
    options: DeleteIntegrationRequestOptions,
  ): Promise<NoDataResponse> {
   ...
   */
  async syncTicket(
    options: SyncTicketRequestOptions,
  ): Promise<DataResponse<SyncResult>> {
    const RELATIVE_URL = `/org/${options.orgId}/tickets/${options.ticketId}/sync/${options.provider}`;

    const response = await this.postRequest<
      DataResponse<SyncResult>,
      Record<string, never>
    >({
      bearerToken: options.bearerToken,
      data: {},
    ...
    return Entity.unMarshal<DataResponse<SyncResult>>(
      response,
      DataResponse<SyncResult>,
    );
  }
   ...
   */
  async unlinkTicket(
    options: UnlinkTicketRequestOptions,
  ): Promise<NoDataResponse> {
   ...
   */
  async bulkSync(
    options: BulkSyncRequestOptions,
  ): Promise<DataResponse<BulkSyncResult>> {
