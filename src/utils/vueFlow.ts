...
export function createAnalyzerNodes(plugins: Plugin[]): { nodes: (AnalyzerNode | ConfigNode)[], edges: Edge[] } {
...
export function layoutNodes(nodes: (AnalyzerNode | ConfigNode)[]): (AnalyzerNode | ConfigNode)[] {
    ...
    // Topological sort to get proper execution order
    const getSortedNodes = (): AnalyzerNode[] => {
        const sorted: AnalyzerNode[] = [];
        const visited = new Set<string>();
        const visiting = new Set<string>();
        
        const visit = (nodeName: string) => {
            if (visited.has(nodeName)) return;
            if (visiting.has(nodeName)) return; // Cycle detection
            ...
            const node = analyzerNodes.find(n => n.data.plugin.name === nodeName);
            if (node) sorted.push(node);
        };
        
        analyzerNodes.forEach(node => visit(node.data.plugin.name));
        return sorted;
    };
