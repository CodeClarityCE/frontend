...
describe("UserAuthForm Accessibility Tests", () => {
    ...
    wrapper = mount(UserAuthForm, {
      global: {
        stubs: {
          ...
          Button: {
            template:
              '<button type="submit" :disabled="disabled"><slot /></button>',
            props: ["disabled"],
          },
          Input: {
            template:
              '<input v-bind="$attrs" :type="type" :name="name" :id="id" :required="required" :aria-describedby="ariaDescribedBy" :aria-label="$attrs[\'aria-label\'] ?? placeholder" :placeholder="placeholder" />',
            props: [
              "type",
              "name",
              "id",
  ...
  describe("Social Authentication Accessibility", () => {
    ...
    it("should indicate button purpose for social logins", async () => {
      ...
      if (googleButton.exists()) {
        const text =
          googleButton.text() ?? googleButton.attributes("aria-label") ?? "";
      ...
      if (githubButton.exists()) {
        const text =
          githubButton.text() ?? githubButton.attributes("aria-label") ?? "";
  ...
  describe.skip("Accessibility Insights and Scoring", () => {
    it("should achieve high accessibility score", async () => {
      ...
      expect(insights.summary.passCount).toBeGreaterThan(
        insights.summary.violationCount,
      );
    });
