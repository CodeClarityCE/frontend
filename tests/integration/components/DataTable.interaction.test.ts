    ...
    wrapper = mount(DataTable, {
      ...
      global: {
        stubs: {
          ...
          TableHead: {
            template:
              '<th class="table-head" @click="$emit(\'click\')"><slot /></th>',
          },
          ...
          SelectTrigger: {
            template:
              '<div class="select-trigger" @click="$emit(\'click\')"><slot /></div>',
          },
          SelectContent: {
          ...
          SelectItem: {
            template:
              '<div class="select-item" @click="$emit(\'select\', value)"><slot /></div>',
            props: ["value"],
            emits: ["select"],
          },
          ...
          DropdownMenuTrigger: {
            template:
              '<div class="dropdown-trigger" @click="$emit(\'click\')"><slot /></div>',
          },
          DropdownMenuContent: {
          ...
          DropdownMenuCheckboxItem: {
            template:
              '<div class="dropdown-checkbox" @click="$emit(\'click\')"><slot /></div>',
            props: ["checked"],
            emits: ["click"],
          },
          FlexRender: {
  ...
  describe("Pagination Controls", () => {
    ...
    it("should navigate between pages", async () => {
      ...
      const nextButton = paginationButtons.find(
        (btn: any) => btn.text().includes("Next") ?? btn.text().includes(">"),
      );
      ...
      if (nextButton) {
        ...
        expect(
          wrapper.vm.table.getState().pagination.pageIndex,
    ...
    it("should toggle column visibility", async () => {
      ...
      if (dropdownTrigger.exists()) {
        ...
        if (columnCheckbox.exists()) {
          ...
          expect(
            Object.keys(wrapper.vm.columnVisibility).length,
          ).toBeGreaterThanOrEqual(0);
        }
      }
    });
  ...
  describe("Row Selection", () => {
    it("should select individual rows", async () => {
      ...
      if (rowCheckboxes.length > 1) {
        ...
        expect(
          Object.keys(wrapper.vm.rowSelection).length,
        ).toBeGreaterThanOrEqual(0);
      }
    });
