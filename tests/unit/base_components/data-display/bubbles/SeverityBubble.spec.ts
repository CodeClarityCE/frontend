...
describe.skip("SeverityBubble", () => {
  ...
  describe("Basic Rendering", () => {
    ...
    it("renders empty when no severity props are provided", () => {
      expect(
        wrapper
          .findAll(".flex.flex-row > div")
          .filter((el: any) => el.text().match(/^[CHML N]$/)),
      ).toHaveLength(0);
    });
  ...
  describe("Multiple Severities", () => {
    it("renders multiple severity boxes when multiple props are true", () => {
      ...
      const classes = severityBoxes.map((box) =>
        box.find(".severity-class").text(),
      );
      expect(classes).toEqual(["C", "H", "L"]);

      const values = severityBoxes.map((box) =>
        box.find(".severity-value").text(),
      );
    ...
    it("applies correct styling when all severities are deactivated", () => {
      ...
      severityBoxes.forEach((box) => {
        ...
        expect(
          severityClass
            .classes()
            .some((cls) => cls.includes("severity-class-critical")),
        ).toBe(false);
        expect(
          severityClass
            .classes()
            .some((cls) => cls.includes("severity-class-high")),
        ).toBe(false);
        expect(
          severityClass
            .classes()
            .some((cls) => cls.includes("severity-class-medium")),
        ).toBe(false);

        expect(
          severityValue
            .classes()
            .some((cls) => cls.includes("severity-value-critical")),
        ).toBe(false);
        expect(
          severityValue
            .classes()
            .some((cls) => cls.includes("severity-value-high")),
        ).toBe(false);
        expect(
          severityValue
            .classes()
            .some((cls) => cls.includes("severity-value-medium")),
        ).toBe(false);
      });
    });
  ...
  describe("Slot Content Handling", () => {
    ...
    it("handles complex slot content", () => {
      ...
      expect(severityValue.html()).toContain(
        '<span class="custom-class">Complex <strong>Content</strong></span>',
      );
    });
  });
