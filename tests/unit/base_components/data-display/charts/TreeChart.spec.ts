...
const createMockNode = (): any => {
  const node = {
    ...
    descendants: vi.fn(() => [
      ...
      {
        data: { id: "child1", uniqueId: "child1" },
        depth: 1,
        x: 40,
        y: 250,
        children: null,
      ...
      {
        data: { id: "child2", uniqueId: "child2" },
        depth: 1,
        x: -40,
        y: 250,
        children: null,
...
describe("TreeChart", () => {
  ...
  describe("Rendering", () => {
    ...
    it("has correct DOM structure", () => {
      ...
      expect(wrapper.find(".tree-chart").attributes("id")).toBe(
        "test-tree-chart",
      );
    });
  ...
  describe("Edge Cases", () => {
    ...
    it("handles dependencies with very long names", () => {
      ...
      expect(wrapper.props().data[0]!.id).toBe(
        "very-long-dependency-name-that-might-cause-layout-issues-in-the-tree-chart-component",
      );
    });
    ...
    it("handles very large dependency trees", () => {
      const largeData: GraphDependency[] = Array.from(
        { length: 100 },
        (_, i) => ({
          id: `dependency-${i}`,
