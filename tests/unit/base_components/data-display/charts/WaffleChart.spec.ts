...
const mockDialogTrigger = {
  template:
    '<div class="mock-dialog-trigger" @click="$emit(\'click\')"><slot /></div>',
  props: ["asChild"],
};
...
vi.mock("d3", () => ({
  ...
  interpolateRdYlBu: vi.fn(
    (t: number) => `hsl(${Math.round(240 * t)}, 70%, 50%)`,
  ),
  interpolateDiscrete: vi.fn(
    (colors: string[]) => (t: number) =>
      colors[Math.floor(t * colors.length)] ?? colors[0],
  ),
  interpolateWarm: vi.fn(
    (t: number) => `hsl(${Math.round(60 * (1 - t))}, 70%, 50%)`,
  ),
  interpolateCool: vi.fn(
    (t: number) => `hsl(${Math.round(240 * t)}, 70%, 50%)`,
  ),
}));
  ...
  describe("Edge Cases", () => {
    ...
    it("handles long label names", () => {
      const longLabelData: WaffleChartEntry[] = [
        {
          label: "Very Long Category Name That Might Cause Layout Issues",
          value: 50,
          color: "#ff0000",
        },
      ];
      ...
      expect(wrapper.props().data[0]!.label).toBe(
    ...
    it("handles very large datasets", () => {
      const largeDataset: WaffleChartEntry[] = Array.from(
        { length: 50 },
        (_, i) => ({
          label: `Category ${i}`,
