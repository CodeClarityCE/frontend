...
describe("Data Display Index Exports", () => {
  ...
  describe("Type and Utility Exports", () => {
    it("exports chart type definitions", async () => {
      ...
      const barChartModule = await import(
        `@/base_components/data-display/charts/barChart`
      );
      expect(barChartModule).toBeDefined();

      const doughnutModule = await import(
        `@/base_components/data-display/charts/doughnutChart`
      );
      expect(doughnutModule).toBeDefined();

      const groupedBarModule = await import(
        `@/base_components/data-display/charts/groupedBarChart`
      );
      expect(groupedBarModule).toBeDefined();

      const radarChartModule = await import(
        `@/base_components/data-display/charts/radarChart`
      );
  ...
  describe("Module Structure", () => {
    ...
    it("does not export unexpected values", async () => {
      ...
      const forbiddenExports = ["default", "__esModule", "internal", "private"];
  ...
  describe("Import Consistency", () => {
    it("direct imports match index exports", async () => {
      ...
      const directBarChart = (
        await import(`@/base_components/data-display/charts/BarChart.vue`)
      ).default;
      const directBubble = (
        await import(
          `@/base_components/data-display/bubbles/BubbleComponent.vue`
        )
      ).default;
      const directTable = (
        await import(`@/base_components/data-display/tables/SortableTable.vue`)
      ).default;
    ...
    it("utility imports match index exports", async () => {
      const indexModule = await import(`@/base_components/data-display`);
      const directUtility = await import(
        `@/base_components/data-display/charts/colors-waffle`
      );

      expect(indexModule.interpolateColors).toBe(
        directUtility.interpolateColors,
      );
    });
  });
