...
describe("ActiveFilterBar", () => {
  ...
  it("removes filter when close button is clicked", async () => {
    ...
    expect(filterState.filterConfig["severity"]!["data"]["high"]!.value).toBe(
      false,
    );
  });
  ...
  it("handles filter state updates correctly", async () => {
    ...
    const updatedFilterState = createMockFilterState([
      mockCheckboxFilter,
      mockRadioFilter,
    ]);
  ...
  it("handles remove button styling for checkbox filters", () => {
    ...
    expect(removeButton.attributes("style")).toContain(
  ...
  it("updates filter state correctly when removing filters", async () => {
    ...
    const originalValue =
      filterState.filterConfig["severity"]!["data"]["high"]!.value;
  ...
  it("handles filter with complex labels", () => {
    const complexFilter: ActiveFilter = {
      label:
        "Very Long Category Name: Very Long Option Name With Special Characters!@#$%",
      category: "complex",
      type: FilterType.CHECKBOX,
