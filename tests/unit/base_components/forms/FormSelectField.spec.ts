...
vi.mock("vee-validate", async () => {
  ...
  return {
    ...
    ErrorMessage: {
      ...
      props: ["name"],
      template:
        '<div data-testid="error-message" class="text-destructive mt-1 block"><slot /></div>',
    },
  };
});
...
describe("FormSelectField", () => {
  describe("Rendering", () => {
    it("renders with basic props", () => {
      ...
      expect(
        screen.getByDisplayValue(defaultProps.placeholder),
      ).toBeInTheDocument();
    ...
    it("renders with custom placeholder", () => {
      ...
      expect(
        screen.getByRole("option", { name: customPlaceholder }),
      ).toBeInTheDocument();
    });
  ...
  describe("Props Validation", () => {
    ...
    it("handles single choice", () => {
      ...
      expect(
        screen.getByRole("option", { name: "Only Option" }),
      ).toBeInTheDocument();
    });
