...
vi.mock("@iconify/vue", () => ({
  Icon: {
    name: "Icon",
    template:
      '<span data-testid="icon" :class="$attrs.class" :data-icon="icon" :title="$attrs.title"><slot /></span>',
    props: ["icon"],
  },
}));
...
describe("ExpandableBox", () => {
  describe("Rendering", () => {
    ...
    it("renders with collapsed state by default", () => {
      ...
      expect(
        wrapper.find(".collapsible-content").attributes("style"),
    ...
    it("has correct container structure", () => {
      ...
      expect(wrapper.find(".project-patch-patch-container").exists()).toBe(
        true,
      );
      expect(
        wrapper.find(".project-patch-patch-container-header").exists(),
      ).toBe(true);
  ...
  describe("Slots", () => {
    ...
    it("renders collapsible_content slot when expanded", () => {
      ...
      expect(wrapper.find(".content-wrapper").html()).toContain(
  ...
  describe("Content Display Logic", () => {
    it("shows collapsibe_content_inverse when collapsed", () => {
      const wrapper = mount(ExpandableBox, {
        props: { expand: false },
        slots: {
          collapsibe_content_inverse:
            '<div data-testid="inverse-content">Collapsed View</div>',
        },
      });
      ...
      const hasDisplayBlock = !parentDiv
        ?.getAttribute("style")
    ...
    it("hides collapsibe_content_inverse when expanded", () => {
      ...
      const inverseContentContainer = wrapper.find(
    ...
    it("shows collapsible_content when expanded", () => {
      const wrapper = mount(ExpandableBox, {
        ...
        slots: {
          collapsible_content:
            '<div data-testid="expandable-content">Expanded View</div>',
        },
      });

      const collapsibleContent = wrapper.find(".collapsible-content");
      expect(collapsibleContent.attributes("style")).toContain(
        "display: block",
      );

      const expandableContent = wrapper.find(
