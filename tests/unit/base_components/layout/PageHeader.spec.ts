...
vi.mock("@iconify/vue", () => ({
  Icon: {
    name: "Icon",
    template:
      '<span data-testid="icon" :class="$attrs.class" :data-icon="icon"><slot /></span>',
    props: ["icon"],
  },
}));
...
vi.mock("@/shadcn/ui/button/Button.vue", () => ({
  default: {
    name: "Button",
    template:
      '<button data-testid="button" :class="$attrs.class" :disabled="disabled ?? undefined" @click="$emit(\'click\')"><slot /></button>',
...
describe("PageHeader", () => {
  ...
  describe("Last Updated Display", () => {
    ...
    it("shows calendar icon in last updated indicator", () => {
      ...
      const calendarIcon = wrapper
        .findAll('[data-testid="icon"]')
        .find(
          (icon) => icon.attributes("data-icon") === "solar:calendar-linear",
        );
  ...
  describe("Refresh Icon", () => {
    it("shows refresh icon when not loading", () => {
      ...
      const refreshIcon = wrapper
        .findAll('[data-testid="icon"]')
        .find(
          (icon) => icon.attributes("data-icon") === "solar:refresh-linear",
        );
    ...
    it("shows loading icon when loading", () => {
      ...
      const loadingIcon = wrapper
        .findAll('[data-testid="icon"]')
        .find(
          (icon) => icon.attributes("data-icon") === "solar:loading-linear",
        );
  ...
  describe("Component Integration", () => {
    it("renders all components together correctly", () => {
      ...
      expect(wrapper.find(".flex.items-center.gap-2.text-sm").exists()).toBe(
        true,
      );
