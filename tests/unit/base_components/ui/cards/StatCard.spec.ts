...
vi.mock("@/shadcn/ui/card", () => ({
  Card: {
    name: "Card",
    template:
      '<div data-testid="card" class="card" :class="$attrs.class"><slot /></div>',
    inheritAttrs: false,
  },
  CardContent: {
    name: "CardContent",
    template:
      '<div data-testid="card-content" class="card-content"><slot /></div>',
  },
}));
...
vi.mock("@iconify/vue", () => ({
  Icon: {
    name: "Icon",
    template:
      '<span data-testid="icon" class="icon" :class="$attrs.class" :data-icon="icon">{{ icon ?? "mock-icon" }}</span>',
  ...
  describe("Slots", () => {
    it("renders subtitle slot content when provided", () => {
      const wrapper = mount(StatCard, {
        ...
        slots: {
          subtitle:
            '<span class="custom-subtitle">Custom subtitle content</span>',
        },
      });
