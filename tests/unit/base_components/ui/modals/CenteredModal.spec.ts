  ...
  describe("Accessibility and UX", () => {
    ...
    it("maintains proper content hierarchy", async () => {
      ...
      expect(children[0]).toMatchObject({
        className: expect.stringContaining("rounded p-5 pb-0"),
      }); // title
      expect(children[1]).toMatchObject({
        className: expect.stringContaining("px-5 py-2"),
      }); // subtitle
      expect(children[2]).toMatchObject({
        className: expect.stringContaining("h-px bg-gray-200"),
      }); // divider
      expect(children[3]).toMatchObject({
        className: expect.stringContaining("p-5"),
      }); // content
      expect(children[4]).toMatchObject({
        className: expect.stringContaining("flex flex-row"),
      }); // buttons
    });
  });
});
