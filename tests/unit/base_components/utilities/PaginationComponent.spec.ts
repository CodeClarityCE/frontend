...
vi.mock("@/shadcn/ui/pagination", () => ({
  ...
  PaginationContent: {
    ...
    template:
      '<div data-testid="pagination-list" class="pagination-list"><slot /></div>',
  },
  PaginationItem: {
    name: "PaginationItem",
    template:
      '<div data-testid="pagination-item" class="pagination-item"><slot /></div>',
    props: ["value", "asChild"],
  },
  PaginationFirst: {
    name: "PaginationFirst",
    template:
      '<button data-testid="pagination-first" class="pagination-first" @click="$emit(\'click\')">First</button>',
    emits: ["click"],
  },
  PaginationPrevious: {
    name: "PaginationPrevious",
    template:
      '<button data-testid="pagination-prev" class="pagination-prev" @click="$emit(\'click\')">Prev</button>',
    emits: ["click"],
  },
  PaginationNext: {
    name: "PaginationNext",
    template:
      '<button data-testid="pagination-next" class="pagination-next" @click="$emit(\'click\')">Next</button>',
    emits: ["click"],
  },
  PaginationLast: {
    name: "PaginationLast",
    template:
      '<button data-testid="pagination-last" class="pagination-last" @click="$emit(\'click\')">Last</button>',
    emits: ["click"],
  },
  PaginationEllipsis: {
    name: "PaginationEllipsis",
    template:
      '<span data-testid="pagination-ellipsis" class="pagination-ellipsis">...</span>',
    props: ["index"],
  },
}));
...
vi.mock("@/shadcn/ui/button", () => ({
  Button: {
    name: "Button",
    template:
      '<button data-testid="button" :class="variant" @click="$emit(\'click\')"><slot /></button>',
...
describe("PaginationComponent", () => {
  ...
  describe("Pagination Controls", () => {
    it("renders all pagination navigation elements", () => {
      ...
      expect(wrapper.find('[data-testid="pagination-first"]').exists()).toBe(
        true,
      );
      expect(wrapper.find('[data-testid="pagination-prev"]').exists()).toBe(
        true,
      );
      expect(wrapper.find('[data-testid="pagination-next"]').exists()).toBe(
        true,
      );
      expect(wrapper.find('[data-testid="pagination-last"]').exists()).toBe(
        true,
      );
    });
    ...
    it("renders pagination structure correctly", () => {
      ...
      expect(wrapper.find('[data-testid="pagination"]').exists()).toBe(true);
      expect(wrapper.find('[data-testid="pagination-list"]').exists()).toBe(
        true,
      );
    });
  });
