...
vi.mock("@iconify/vue", () => ({
  Icon: {
    name: "Icon",
    template:
      '<div data-testid="icon" :class="[$attrs.class, icon]" @click="$emit(\'click\')">{{ icon }}</div>',
    props: ["icon"],
    emits: ["click"],
  },
}));
...
vi.mock("@/shadcn/ui/select", () => ({
  ...
  SelectItem: {
    name: "SelectItem",
    template:
      '<div data-testid="select-item" :data-value="value" @click="$emit(\'click\')"><slot /></div>',
    props: ["value"],
    emits: ["click"],
  },
  SelectLabel: {
...
describe("UtilitiesSort", () => {
  ...
  const createWrapper = (
    ...
    return mount(UtilitiesSort, {
      props: {
        ...
        "onUpdate:pageLimitSelected":
          modelValues["onUpdate:pageLimitSelected"] ??
          vi.fn<(value: number) => void>(),
        sortKey: modelValues.sortKey ?? "",
        "onUpdate:sortKey":
          modelValues["onUpdate:sortKey"] ?? vi.fn<(value: string) => void>(),
        sortDirection: modelValues.sortDirection ?? SortDirection.DESC,
        "onUpdate:sortDirection":
          modelValues["onUpdate:sortDirection"] ??
          vi.fn<(value: SortDirection) => void>(),
      },
    });
  };
  ...
  describe("Component Rendering", () => {
    it("renders the main container with correct structure", () => {
      ...
      expect(
        wrapper
          .find(".flex.flex-row.gap-1.justify-between.items-center")
          .exists(),
      ).toBe(true);
  ...
  describe("Edge Cases", () => {
    ...
    it("handles sort option with special characters", () => {
      ...
      const specialItem = allItems.find(
        (item) => item.text() === "Special & Characters",
      );
