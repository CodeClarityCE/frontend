...
vi.mock(
  "@/codeclarity_components/authentication/oauth/OAuth2Callback.vue",
  async () => {
    const { defineComponent } = await import("vue");
    return {
      default: defineComponent({
        ...
        props: ["provider"],
        template:
          '<div data-testid="oauth2-callback" :data-provider="provider">OAuth2 Callback Component</div>',
      }),
      __isTeleport: false,
      __isKeepAlive: false,
    };
...
describe("OAuthCallbackView", () => {
  ...
  describe("Props Handling", () => {
    it("should pass GitLab provider to OAuth2Callback component", () => {
      ...
      const oauth2Component = wrapper.find('[data-testid="oauth2-callback"]');
      expect(oauth2Component.attributes("data-provider")).toBe(
  ...
  describe("Component Integration", () => {
    ...
    it("should pass provider prop correctly to child component", () => {
      ...
      const oauth2Component = wrapper.findComponent(
        '[data-testid="oauth2-callback"]',
      );
      expect(oauth2Component.exists()).toBe(true);
      expect((oauth2Component as any).props("provider")).toBe(
        SocialProvider.GITHUB,
      );
    });
  });
  ...
  describe("TypeScript Integration", () => {
    it("should properly type the provider prop", () => {
      ...
      expect(Object.values(SocialProvider)).toContain(
        wrapper.props("provider"),
      );
    });
