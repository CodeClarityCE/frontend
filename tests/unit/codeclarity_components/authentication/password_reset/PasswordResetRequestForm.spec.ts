...
vi.mock("vue-router", async () => {
  ...
  return {
    RouterLink: defineComponent({
      ...
      props: ["to"],
      template:
        '<a :href="to.name" data-testid="router-link"><slot></slot></a>',
    }),
  };
});
  ...
  describe("Component Structure", () => {
    it("should render the main container", () => {
      ...
      const container = wrapper.find(
        ".flex.flex-col.justify-center.items-center.my-20",
      );
  ...
  describe("Error Handling", () => {
    it("should display error message on ValidationError", async () => {
      const validationError = new ValidationError(
        "validation-error",
        "Validation failed",
        [{ property: "email", errors: ["Invalid email"] }],
      );
      mockAuthRepository.requestPasswordReset.mockRejectedValue(
        validationError,
      );
      ...
      const errorIcon = wrapper.find(
        '[data-icon="material-symbols:error-outline"]',
      );
    ...
    it("should reset error state on new submission", async () => {
      mockAuthRepository.requestPasswordReset.mockRejectedValueOnce(
        new Error("Error"),
      );
  ...
  describe("Success State", () => {
    ...
    it("should display proper success instructions", async () => {
      ...
      expect(instructions.text()).toContain(
        "If a user is registered under that email",
      );
