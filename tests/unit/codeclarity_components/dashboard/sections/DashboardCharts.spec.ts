...
vi.mock("@/base_components/ui/cards/InfoCard.vue", () => ({
  default: {
    name: "InfoCard",
    template:
      '<div data-testid="info-card" :class="$attrs.class"><slot /></div>',
...
vi.mock(
  "@/codeclarity_components/dashboard/charts/ExposureOverview.vue",
  () => ({
    default: {
...
vi.mock(
  "@/codeclarity_components/dashboard/charts/VulnerabilityImpact.vue",
  () => ({
    default: {
      name: "VulnerabilityImpact",
      template:
        '<div data-testid="vulnerability-impact">VulnerabilityImpact</div>',
      props: ["integrationIds"],
    },
  }),
);
...
describe("DashboardCharts", () => {
  ...
  it("should render all chart components", () => {
    ...
    expect(wrapper.find('[data-testid="exposure-overview"]').exists()).toBe(
      true,
    );
    ...
    expect(wrapper.find('[data-testid="vulnerability-impact"]').exists()).toBe(
      true,
    );
  });
  ...
  it("should pass integration IDs to all chart components", () => {
    ...
    const exposureOverview = wrapper.findComponent({
      name: "ExposureOverview",
    });
    const currentVulns = wrapper.findComponent({ name: "CurrentVulns" });
    const licenseDist = wrapper.findComponent({ name: "LicenseDist" });
    const vulnerabilityImpact = wrapper.findComponent({
      name: "VulnerabilityImpact",
    });

    expect(exposureOverview.props("integrationIds")).toEqual(
      defaultProps.integrationIds,
    );
    expect(currentVulns.props("integrationIds")).toEqual(
      defaultProps.integrationIds,
    );
    expect(licenseDist.props("integrationIds")).toEqual(
      defaultProps.integrationIds,
    );
    expect(vulnerabilityImpact.props("integrationIds")).toEqual(
      defaultProps.integrationIds,
    );
  });
