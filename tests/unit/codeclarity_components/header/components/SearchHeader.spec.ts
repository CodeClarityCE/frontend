...
const mockCommandDialog = {
  name: "CommandDialog",
  template:
    '<div data-testid="command-dialog" style="display: none;"><slot /></div>',
  props: ["open"],
  emits: ["update:open"],
};
...
const mockCommandInput = {
  name: "CommandInput",
  template:
    '<input data-testid="command-input" placeholder="Type a command or search..." style="display: none;" />',
  props: ["placeholder"],
};
...
const mockCommandList = {
  name: "CommandList",
  template:
    '<div data-testid="command-list" style="display: none;"><slot /></div>',
  emits: ["click"],
};
...
const mockCommandEmpty = {
  name: "CommandEmpty",
  template:
    '<div data-testid="command-empty" style="display: none;">No results found.</div>',
};

const mockCommandGroup = {
  name: "CommandGroup",
  template:
    '<div data-testid="command-group" style="display: none;"><slot /></div>',
  props: ["heading"],
};
...
const mockCommandItem = {
  name: "CommandItem",
  template:
    '<div data-testid="command-item" style="display: none;"><slot /></div>',
  props: ["value", "class"],
};
...
describe("SearchHeader - Simplified", () => {
  ...
  describe("Component Structure", () => {
    ...
    it("renders search icon in button", () => {
      ...
      const searchIcon = icons.find(
        (icon: any) => icon.props("icon") === "lucide:search",
      );
  ...
  describe("Edge Cases", () => {
    ...
    it("renders all required elements", () => {
      ...
      expect(wrapper.findComponent({ name: "CommandDialog" }).exists()).toBe(
        true,
      );
