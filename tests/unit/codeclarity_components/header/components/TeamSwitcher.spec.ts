...
const mockButton = {
  name: "Button",
  template:
    '<button role="combobox" aria-expanded="open" aria-label="Select an organization" v-bind="$attrs" @click="$emit(\'click\')"><slot /></button>',
...
const mockPopoverContent = {
  name: "PopoverContent",
  template:
    '<div data-testid="popover-content" v-bind="$attrs" style="display: none;"><slot /></div>',
  props: ["class"],
};
...
const mockCommandEmpty = {
  name: "CommandEmpty",
  template:
    '<div data-testid="command-empty" style="display: none;"><slot /></div>',
};
...
const mockAvatarImage = {
  name: "AvatarImage",
  template:
    '<img data-testid="avatar-image" alt="Test Avatar" v-bind="$attrs" />',
  props: ["src", "alt", "class"],
};
