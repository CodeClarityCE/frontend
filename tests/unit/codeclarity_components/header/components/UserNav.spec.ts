...
vi.mock("@iconify/vue", () => ({
  Icon: {
    name: "Icon",
    template:
      '<span data-testid="icon" :data-icon="icon" v-bind="$attrs"></span>',
    props: ["icon", "class"],
  },
}));
...
const mockDropdownMenuTrigger = {
  ...
  template:
    '<div data-testid="dropdown-trigger" v-bind="$attrs"><slot /></div>',
  props: ["as-child"],
};
...
const mockDropdownMenuContent = {
  ...
  template:
    '<div data-testid="dropdown-content" style="display: none;" v-bind="$attrs"><slot /></div>',
  props: ["class", "align"],
};
...
const mockDropdownMenuLabel = {
  ...
  template:
    '<div data-testid="dropdown-label" style="display: none;" v-bind="$attrs">johndoe john@example.com</div>',
  props: ["class"],
};
...
const mockDropdownMenuGroup = {
  ...
  template:
    '<div data-testid="dropdown-group" style="display: none;"><slot /></div>',
};

const mockDropdownMenuItem = {
  name: "DropdownMenuItem",
  template:
    '<div data-testid="dropdown-item" style="display: none;" v-bind="$attrs" @click="$emit(\'click\')"><slot /></div>',
  props: ["class"],
  emits: ["click"],
};
...
const mockDropdownMenuShortcut = {
  ...
  template:
    '<span data-testid="dropdown-shortcut" style="display: none;"><slot /></span>',
};
...
const mockDialogTrigger = {
  name: "DialogTrigger",
  template:
    '<div data-testid="dialog-trigger" style="display: none;"><slot /></div>',
};

const mockDialogContent = {
  name: "DialogContent",
  template:
    '<div data-testid="dialog-content" style="display: none;"><slot /></div>',
};

const mockDialogTitle = {
  name: "DialogTitle",
  template:
    '<h2 data-testid="dialog-title" style="display: none;">Notifications</h2>',
};

const mockDialogDescription = {
  name: "DialogDescription",
  template:
    '<div data-testid="dialog-description" style="display: none;">You have 2 new notifications</div>',
};

const mockDialogFooter = {
  name: "DialogFooter",
  template:
    '<div data-testid="dialog-footer" style="display: none;"><slot /></div>',
};
...
const mockRouterLink = {
  name: "RouterLink",
  template:
    '<a data-testid="router-link" style="display: none;" v-bind="$attrs">Profile</a>',
  props: ["to", "title"],
};
...
describe("UserNav - Simplified", () => {
  ...
  describe("Component Structure", () => {
    ...
    it("renders dropdown menu structure", () => {
      ...
      const dropdownTrigger = wrapper.findComponent({
        name: "DropdownMenuTrigger",
      });
      const dropdownContent = wrapper.findComponent({
        name: "DropdownMenuContent",
      });
  ...
  describe("Component Integration", () => {
    ...
    it("properly integrates dropdown menu components", () => {
      const dropdownMenu = wrapper.findComponent({ name: "DropdownMenu" });
      const trigger = dropdownMenu.findComponent({
        name: "DropdownMenuTrigger",
      });
      const content = dropdownMenu.findComponent({
        name: "DropdownMenuContent",
      });
