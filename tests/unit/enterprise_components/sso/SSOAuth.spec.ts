...
vi.mock("@/shadcn/ui/button", async () => {
  ...
  return {
    Button: defineComponent({
      ...
      emits: ["click"],
      template:
        '<button :type="type" :data-variant="variant" @click="$emit(\'click\')" data-testid="sso-button"><slot></slot></button>',
    }),
  };
});
...
describe.skip("SSOAuth", () => {
  ...
  describe("Component Structure", () => {
    ...
    it("should render GitLab authentication button", () => {
      ...
      const buttons = wrapper.findAll('[data-testid="sso-button"]');
      const gitlabButton = buttons.find((button) =>
        button.text().includes("GitLab"),
      );
  ...
  describe("GitHub Authentication", () => {
    it("should initiate GitHub authentication when GitHub button is clicked", async () => {
      ...
      expect(mockAuthStore.setSocialAuthState).toHaveBeenCalledWith(
        "mock-oauth-state-123",
      );
    });
    ...
    it("should construct correct GitHub authentication URL", async () => {
      ...
      expect(window.location.href).toBe(
        "https://localhost/api/auth/github/authenticate?state=mock-oauth-state-123",
      );
    });
    ...
    it("should handle different hostname in GitHub URL construction", async () => {
      ...
      expect(window.location.href).toBe(
        "https://example.com/api/auth/github/authenticate?state=mock-oauth-state-123",
      );
    });
  ...
  describe("GitLab Authentication", () => {
    ...
    it("should construct correct GitLab authentication URL", async () => {
      ...
      expect(window.location.href).toBe(
        "https://localhost/api/auth/gitlab/authenticate?state=mock-oauth-state-123",
      );
    });
    ...
    it("should handle different hostname in GitLab URL construction", async () => {
      ...
      expect(window.location.href).toBe(
        "https://example.com/api/auth/gitlab/authenticate?state=mock-oauth-state-123",
      );
    });
  ...
  describe("URL Construction", () => {
    it("should properly encode state parameter in URL", async () => {
      ...
      expect(window.location.href).toContain(
        "state=state%2Bwith%2Fspecial%26chars",
      );
    });
