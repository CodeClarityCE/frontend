...
export class TestAnalyticsCollector {
  ...
  getSuiteAnalytics(suiteName: string): SuiteAnalytics {
    ...
    const sortedByDuration = [...suiteMetrics].sort(
      (a, b) => b.duration - a.duration,
    );
    const slowestTest = sortedByDuration[0]?.testName ?? "";
    const fastestTest =
      sortedByDuration[sortedByDuration.length - 1]?.testName ?? "";
  ...
  getOverallAnalytics(): {
    ...
    const sortedSuites = suiteAnalytics.sort(
      (a, b) => b.analytics.averageDuration - a.analytics.averageDuration,
    );
  ...
  getFailurePatterns(): {
    ...
    return Array.from(grouped.entries())
      .map(([key, metrics]) => {
        ...
        const errorPatterns = [
          ...new Set(metrics.map((m) => m.error ?? "").filter((e) => e)),
        ];
  ...
  generateTrendReport(days = 30): {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const averageDuration =
        dayMetrics.reduce((sum, m) => sum + m.duration, 0) / totalTests;
      const memoryUsage =
        dayMetrics
          .map((m) => m.memoryUsage?.usedJSHeapSize ?? 0)
  ...
  getTestQualityScore(): {
    ...
    const totalScore =
      successRateScore + performanceScore + stabilityScore + coverageScore;

    const recommendations = [];
    if (analytics.successRate < 95) {
      recommendations.push(
        "Improve test reliability - success rate is below 95%",
      );
    }
    if (slowTests.length > 10) {
      recommendations.push(
        "Optimize slow tests - consider mocking or reducing test scope",
      );
    }
  ...
  private getFlakyTestsForSuite(suiteName: string): string[] {
    return Array.from(this.flakyTestHistory.values())
      .filter(
        (data): boolean =>
          data.suiteName === suiteName && data.failureRate > 10,
      )
  ...
  private getMemoryUsage(): MemoryInfo | undefined {
    if (typeof window !== "undefined") {
      const perfWithMemory = window.performance as unknown as {
        memory?: MemoryInfo;
      };
...
export class CIAnalyticsIntegration {
  static generateJUnitReport(metrics: TestMetrics[]): string {
    ...
    for (const [suiteName, tests] of suites) {
      const totalTime =
        tests.reduce((sum, test) => sum + test.duration, 0) / 1000; // Convert to seconds
